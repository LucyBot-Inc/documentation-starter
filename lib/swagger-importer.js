...
Swagger.importActions = function(spec, options) {
  ...
  var includeBearer = false;
  for (def in spec.securityDefinitions) {
    if (spec.securityDefinitions[def].type === 'oauth2') includeBearer = true;
  }
  ...
  paths.forEach(function(path) {
    ...
    for (var i = 0; i < verbs.length; ++i) {
      ...
      var headers = {};
      var body = {};
      if (includeBearer) headers['Authorization'] = {join: ['Bearer ', {answer: 'oauth2'}]}
      ...
      if (options.proxy) {
        ...
        var newPath = Path.join('proxy', actionOpts.protocol, actionOpts.domain);
        if (typeof actionOpts.path === 'string') {
          actionOpts.path = Path.join(newPath, actionOpts.path);
      ...
      actions.push(Utils.createAction(actionOpts, spec.paths[path][verb].operationId || verb + path))
    }
  })
  return actions;
}
